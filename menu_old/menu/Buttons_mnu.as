package
{
	...
	import flash.events.*;
  public class Buttons_mnu extends MovieClip{
	...
	public function Buttons_mnu(menu_attr, menu_over_attr, sub_attr, sub_over_att, snd_att):void{
	...
	public function add_menu_item(menu_item:Array){
		...
		for(i=0;i<menu_item.length;i++){
			//trace(menu_item[i]);			  //add menu item
			tmp = new Array();
			...
			if(menu_item[i].length == 3){
				...
				var nSprite = new Sprite();	//object container
				...
				for(j=0;j<menu_item[i][2].length;j++){
				  ...
				  add_sub_button(menu_item[i][2][j][0], 0, j, nSprite); //trace(tmp); //tmp contains the objects from the call
				}
				//trace(tmp);
				//trace(nSprite.numChildren);
		...
		//trace(obj_ary[0][0].y); trace(obj_ary[0][0].height);
	...
	public function add_button(aLabel:String, Y:int, aryOffset:int){
		...
		//tab0.scaleX = 2; tab0.scaleY = 2;					// resize the button
		...
		tab0 = tabSprite(tabScheme, false);					// generate the button
		tab0.x = W;	tab0.y = Y;								// move the button
		//tab0.scaleX = 2; tab0.scaleY = 2;					// resize the button
		tab0.name = "MOSprite"+aryOffset;
		tmp[tmp.length] = tab0;
		tab0.addEventListener(MouseEvent.MOUSE_DOWN, mouseDown_evt); //click listener

		W += Offset+tab0.width;
	}
	
	public function add_sub_button(aLabel:String, Y:int, aryOffset:int, container){
		...
		box0.x = subW;	box0.y = Y;								// move the button
		//box0.scaleX = 2; box0.scaleY = 2;					// resize the button
		box0.name = "SMSprite"+aryOffset;		
		//tmp[tmp.length] = box0;							//keep object in the array
		...
		box0 = boxSprite(boxScheme, false);					// generate the button
		box0.x = subW;	box0.y = Y;							// move the button
		//box0.scaleX = 2; box0.scaleY = 2;					// resize the button
		box0.name = "SMOSprite"+aryOffset;
		//tmp[tmp.length] = box0;							//keep object in the array
		container.addChild(box0);
		box0.addEventListener(MouseEvent.MOUSE_DOWN, mouseDown_evt); //click listener
		subW += Offset+box0.width;		
	}
	...
	private function mouseOver_evt(event:MouseEvent):void {
		//trace("trace--------"+event.target.parent.name+"------------"+event.target.parent);
		...
		//var i = tLink.charAt(tLink.length-1); //change this to use reg ex, to account for more thatn 10 links
		var i = tLink.match(pat); //trace(i);
		if( tLink.search("MSprite") == 0 ){ // this statement fixes anomily for mouseover stage,
			...
			if(m_visible != null){
				...
				if(obj_ary[m_visible].length == 4){ //if has submenu
					...
					if (obj_ary[m_visible][3][0].numChildren >= (sm_visible*2)+2 && sm_visible != null){
							...
							//if one submenu item was selected unselect
						//trace("MSprite set false");							
						//trace("sm_visible:"+sm_visible+" m_visible:"+m_visible+"[3][0].numchildren "+obj_ary[m_visible][3][0].numChildren);
						sm_visible = null;
					}
				}
			}
			...
			if(obj_ary[i].length == 4){				//submenu visability
				...
				bar.height = obj_ary[i][3][0].height + 8;	//adjust the bar height for the currently displayed submenu
				//trace(bar.y); trace(obj_ary[i][3][0].height);
			}
			m_visible = i;
			...
			if (obj_ary[m_visible][3][0].numChildren >= (sm_visible*2)+2 && sm_visible != null){
				obj_ary[m_visible][3][0].getChildAt((sm_visible*2)+1).visible = false;
				//trace("SMSprite set false");
				//trace("sm_visible:"+sm_visible+" m_visible:"+m_visible+"[3][0].numchildren "+obj_ary[m_visible][3][0].numChildren);
			}
			obj_ary[m_visible][3][0].getChildAt((i*2)+1).visible = true;
			sm_visible = i;
		}
		//trace(event.target.parent.name); 	trace(i);		
	}
